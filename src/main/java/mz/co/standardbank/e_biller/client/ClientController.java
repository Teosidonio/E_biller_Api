...
public class ClientController {
	...
	public ResponseEntity<Client> subscribe ( @PathVariable String contract ) {
		try {
			return new ResponseEntity<>(service.save(contract) , HttpStatus.OK);
	...
	public ResponseEntity<Client> unsubscribe ( @PathVariable String contract ) {
		try {
			return new ResponseEntity<>(service.setActive(contract , false) , HttpStatus.OK);
	...
	public ResponseEntity<Client> registerBiller ( @PathVariable String contract , @PathVariable String biller ) {
		try {
			return new ResponseEntity<>(service.registerBiller(contract , biller) , HttpStatus.OK);
		} catch ( EntityNotFoundException e ) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND , e.getMessage());
		} catch ( IllegalArgumentException e ) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST , e.getMessage());
	...
	public ResponseEntity<Client> unregisterBiller ( @PathVariable String contract , @PathVariable String biller ) {
		try {
			return new ResponseEntity<>(service.unregisterBiller(contract , biller) , HttpStatus.OK);
		} catch ( EntityNotFoundException e ) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND , e.getMessage());
		} catch ( IllegalArgumentException e ) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST , e.getMessage());
	 ...
	 */
	@GetMapping( "/find/all" )
	public ResponseEntity<Page<Client>> findAll ( @RequestParam int page , @RequestParam int size ) {
		try {
	...
	public ResponseEntity<Client> findByContract ( @PathVariable String contract ) {
		try {
			return new ResponseEntity<>(service.find(contract) , HttpStatus.OK);
		} catch ( EntityNotFoundException e ) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND , e.getMessage());
		} catch ( Exception e ) {
			throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR , e.getMessage());
